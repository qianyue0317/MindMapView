 ...
 * @since 2023/9/19
 */
class BothSideLinePainter(val context: Context) : NodeLinePainter {

    private val _path: Path = Path()

    private val cornerR: Float =
        context.resources.getDimensionPixelOffset(R.dimen.mind_map_line_corner_radius).toFloat()

    private val _linePaint = Paint().apply {
        strokeWidth =
            context.resources.getDimensionPixelOffset(R.dimen.mind_map_line_stroke_width).toFloat()
    ...
    override fun drawLine(canvas: Canvas, nodeView: View, parentNodeView: View) {
        ...
        _path.apply {
            if (parentNodeView.left < nodeView.left) {
                ...
                val dx = nodeView.right - parentNodeView.left
                val dy =
                    nodeView.top + nodeView.measuredHeight / 2 - (parentNodeView.top + parentNodeView.measuredHeight / 2)

                moveTo(
                    parentNodeView.left.toFloat().apply { currentX = this },
                    (parentNodeView.top + parentNodeView.measuredHeight / 2).toFloat()
                        .apply { currentY = this }
                )
